...
| 6.5 | Evidence artifacts (deterministic concrete chart/document/store/product/checklist anchors) | `evidenceArtifactAgent.ts` |
...
- **Evidence artifact layer**: `EvidenceArtifactAgent` runs before visual planning and deterministically assigns each slide a concrete human-editor artifact (`PRICE_CHART`, `EARNINGS_TABLE`, `FILING_EXCERPT`, `STORE_SCENE`, `PRODUCT_DETAIL`, `EXECUTIVE_CONTEXT`, `SOCIAL_SCREENSHOT`, `CHECKLIST_RECEIPT`, `SOURCE_DOCUMENT`, `RISK_MATRIX`). Ticker profiles currently include NVDA, LULU, AAPL, AMZN, MSFT, and GOOGL, with fallbacks for other tickers. This is the guard against fake portfolio managers, unrelated logo collages, and bull/rocket/chess/missile filler.
- **Locked visual grammar**: `VisualPlanAgent` (deterministic, side-effect-free) converts policy into a complete slide-by-slide `VisualPlan` — per-slide `visualStyle`, `visualPosition`, `role`, `bucket`, `dominantSubjectClass`, `layoutArchetype`, `primaryEncoding`, `sceneConceptId`, `evidenceArtifact`, plus `StoryboardContinuity` (premise, anchor, shared invariants, progression/resolution rules, evidence artifact plan) and a `compositionSignature`. It enforces no-adjacent-bucket / no-adjacent-dominant-subject, human caps, required chart slides, and payoff placement.
...
`promptLibrary.ts` is the canonical source for `ViralStyle`. ~50 templates grouped into: Data (charts), Cinematic/Metaphor, Human Elements, Typography/Layouts, plus 17 v2 specialized financial templates (CANDLESTICK_HERO, CAP_TABLE_GRID, INSTITUTIONAL_FLOW_SANKEY, EARNINGS_HEAT_TABLE, TYPOGRAPHIC_MEGA_NUMBER, EDITORIAL_REACTION_CARICATURE, etc.). `EXCLUDED_FROM_ROTATION` keeps low-quality or AI-tell styles out of default rotation — including missile/rocket/racing/chess/bull metaphors, generic logo orbits, fake executive lineups, old globe/vault/luxury CTA tropes, and synthetic HUD/gaming looks. Excluded styles remain in the library for explicit, topic-justified override but never appear in normal rotation. The LUXURY_LIFESTYLE scene variants were rewritten away from the jet/Rolex/yacht monoculture toward fresher real-life premium scenes.
...
**ChatGPT Images 2.0** is the PRIMARY manual render target; **Seedream** is the cinematic image-first alternate (the old `Seedance` label was a misnomer and is corrected). Each compiled prompt is a labeled-section packet (`DELIVERABLE`, `STORYBOARD CONTINUITY`, `MUST KEEP`, `CHANGE ONLY`, `EDITORIAL REALISM CONTRACT`, `EVIDENCE ARTIFACT`, `PURPOSE`, `INFORMATION ARCHITECTURE`, `SUBJECT AND SCENE`, `COMPOSITION`, `EXACT TEXT`, `DATA GRAPHIC SPECIFICATION`, `STYLE AND MATERIALS`, `NEGATIVE CONSTRAINTS`) designed for one ChatGPT conversation: paste the slide-1 anchor, then continue in the same chat keeping the `MUST KEEP` invariants. The editorial realism contract plus explicit evidence artifact section forces each slide toward real-world evidence anchors (filings, source-style charts, product/store scenes, named public figures when appropriate), bans unrelated company logos/logo orbits and fake generic finance people, and injects ticker-specific anchors such as NVDA/Jensen/GPU/data-center references or LULU/storefront/apparel/inventory/earnings references. `modelRecommendation.ts` tags each slide; `imagePromptAgent.ts` seeds scene variants with `${date}-s${slot}` so same-day slots don't collapse onto the same scene. No Seedream/Seedance API is integrated — prompts are pasted manually. NOT optimized for Gemini Imagen or Midjourney; the LLM prompt forbids Midjourney CLI flags.
...
- `EvidenceArtifactAgent` is deterministic and side-effect-free. Any new real-world topic/ticker anchor (chart, filing, store/product detail, public executive context, risk matrix) belongs there before being consumed by `VisualPlanAgent` and `ImagePromptAgent`.
